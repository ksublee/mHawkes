...
BETA2 <- matrix(rep(2.55, 4), nrow=2, byrow=TRUE)
#ETA2 <- matrix(rep(.0, 4), nrow=2)


mHSpec2 <- new("mHSpec", MU=MU2, ALPHA=ALPHA2, BETA=BETA2)
L0 <- matrix(c(0.03,0.03,0.03,0.03), nrow=2, byrow=TRUE)

res2 <- mHSim(mHSpec2, LAMBDA0=L0, n=10000)
...
mle <- mHFit(mHSpec2, inter_arrival = inter_arrival, jump_type = jump_type)
print(summary(mle))
